...
{	
	...
	public function create_notice($fromuser, $touser, $point, $typeorder, $typewallet){
	    if (Auth::check()){
	        $notice = new Notice();
	        $notice->notice_fromuser = $fromuser;
	        $notice->notice_touser = $touser;
	        $notice->notice_point = $point;
	        $notice->notice_typeorder = $typeorder;
	        $notice->notice_typewallet = $typewallet;
	...
	public function sendOTPM(Request $request){
		if (Auth::check()){
			$otp = mt_rand(100000,999999);
            
		
		return response()->json(['otpM'=>$otp]);
		}else{
	...
	public function postPTTT(Request $request){
		...
		if(count($array_F1_qualified) == 5){
            ...
            $walletmainController->create_notice($id, $id, 100, 5, 3);
				...
				if ($transfer_status == 0){
					if($created <= $today ){
						...
						$walletmainController->createWalletHistory($transfer_point, $id, $id, $eco_wallet->eco_wallet_id , 2, 1 ,6);
                          $walletmainController->create_notice($id, $id, $transfer_point * 3, 6, 2);
                          $walletmainController->create_notice($id, $id, $transfer_point, 6, 0);
			...
			if($temp > 0){
				if($temp > ($rate * $eco_wallet_point)){
					...
					$walletmainController->createWalletHistory($rate * $eco_wallet_point, $id, $user_introduction, $wallet_ext->ext_wallet_id, 1, 1 ,4);
					 $walletmainController->create_notice($id, $user_introduction, $rate * $eco_wallet_point, 4, 1);
					...
					$walletmainController->createWalletHistory($temp, $id, $user_introduction, $wallet_ext->ext_wallet_id, 1, 1 ,4);
					 $walletmainController->create_notice($id, $user_introduction, $temp, 4,1);
   ...
   {			
   		...
   		if(Hash::check($request->pwdtt,$passWord)){
	        ...
	        //insert to database
		    $transactionConntroller = new TransactionController(); 
			$transactionConntroller->createTransaction(1, $id, $id, null ,$request->currency , $request->point, $request->description, $url_phieuCK, $url_phieuCK2, 0);
	 ...
	 public function RutTransaction(Request $request){				
   		...
   		if (($request->point <= $walletmain_point) && ($request->point >= 100)){
   			if(Hash::check($request->pwdtt,$passWord)){
				   ...
				   $transactionConntroller = new TransactionController(); 
				   $transactionConntroller->createTransaction(0, $id, $id, null ,$request->currency, $request->point, $request->description, null, null, 0);

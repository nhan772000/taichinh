...
class UsersController extends Controller
{
    public function readNotice(Request $request){
         if (Auth::check()){
            Notice::where('id', $request->id)->update(['notice_read_status'=> 1]);
        return null;
    ...
    if(Auth::check()){
      $user = Auth::user();
      $listNotice = DB::table('notice')->where('notice_fromuser', $user->id)->orWhere('notice_touser', $user->id)->orderBy('id','desc')->paginate(10);
      
     return view('notice',['listNotice'=>$listNotice]); 
    }
    }
  ...
  public function postChuyen(Request $request){
    ...
    if(Auth::check()){
      ...
      $this->validate($request, [
        ...
        'id_user_transfer' => ['required', 'max:255', 'regex:^[a-z]+$^'],
     ...
     $user_tranfer_point1 = User::where('id', $request->id_user_transfer)->orwhere('name', $request->id_user_transfer)->first();
      ...
      $user_transfer_point = User::find($user_tranfer_point1->id);
        ...
        if($wallet_point > 1 && $wallet_point > $request->point_transfer && $request->point_transfer > 1){
   ...
   public function getSettingCashBack(){
      if(Auth::check()){
        $user = Auth::user();
        $user_setting = SettingTransferPoint::where('setting_ofuser', $user->id)->first();
        return view('member.setting')->with('type_setting', $user_setting->setting_type);
      }
  }
  public function postSettingCashBack(Request $request){
    //user_type_cashback
    if(Auth::check()){
        $user = Auth::user();
        $user_setting = SettingTransferPoint::where('setting_ofuser', $user->id)->update(['setting_type' => $request->user_type_cashback] );
...
// ----------------Xử lý đăng nhập, đăng ký, đăng xuất người dùng ------------
  //phát triển thị trường
  public function marketDevelopment(Request $request){
  ...
  public function register(Request $request){
    if(Auth::check()){ 
      ...
      return view('auth.register',['user_introduction'=> $request->user_introduction]);
    }
        ...
        if(Auth::attempt(['name' => $user_username, 'password' => $user_password]) || Auth::attempt(['id' => $user_username, 'password' => $user_password]))
        {
          if(Auth::check()) {
            ...
            User::where('id',$id)->update([
              'datelogin' => $datetimeNow
            ]);
            $daylogin = User::where('id',$id)->value('datelogin');
            if( $daylogin > $daygift && $daygift < $daylogin ){
